# PointFocus solver: constant and 1/r amplitude

Implemented **2026-09-21** and expanded to multiple Point and Mesh targets. The CPU backend generates one phase-only SLM pattern by coherently summing the complex contribution from every target emitter. `PointFocus` keeps the existing constant-amplitude calculation and remains the default. **Point Focus (1/r Amplitude)** (`PointFocusInverseR`) weights each emitter by the inverse of its distance to each SLM pixel. Both modes accept one or more targets and run on CPU or Docker/CUDA. A Point target supplies one emitter, and a Mesh target supplies its sampled point cloud. The workbench supplies SI scene descriptions and the solver publishes through the existing phase API and selected-SLM preview.
...
k = 2*pi / WavelengthM
r_j = length(q_j - p)
PointFocus:         F(p) = sum_j a_j       * exp(i * (phi_j - k*r_j))
PointFocusInverseR: F(p) = sum_j (a_j/r_j) * exp(i * (phi_j - k*r_j))
phase_incident(p) = InitialPhaseRad + k * dot(DirectionSLM, p)
phase_slm(p) = wrap_[0, 2*pi)(atan2(Im(F(p)), Re(F(p))) - phase_incident(p))
```

The sum is over **complex fields, not phase angles**. Accumulate weighted cosine and sine contributions, then take `atan2` once for the resulting field. The inverse-distance mode uses **field amplitude `a_j/r_j`**, with `r_j` in meters; it does not use intensity weighting `a_j/r_j^2`. Distance is measured separately from every emitter to every SLM pixel. A common one-meter reference factor would cancel from the phase. A single positive emitter therefore produces exactly the same phase pattern in both modes; several emitters at different distances generally change their relative contributions and the resulting phase. At a pixel where all emitter distances are equal, the modes agree apart from floating-point rounding. Numerically, the implementation precomputes emitter phasors and rotates them by `exp(-i*k*r_j)`; this preserves small relative target-phase offsets when the propagation phase is large. For one positive-amplitude Point target this reduces to the original `wrap(target_phase - k*r - phase_incident)`. Adding incident phase and `+k*r` then gives that single target's phase modulo `2*pi`. With multiple emitters, the output retains only the summed field's phase; it does not guarantee each target's requested intensity or eliminate interference between reconstructed points.
...
`ACGHSolverActor` owns job state, request identifiers, cancellation, queued input, and accepted publication. `UCGHSolverBackend::Submit()` is the replaceable backend boundary. `UCGHCPUSolverBackend` schedules the pure numerical calculation on Unreal's thread pool. `UCGHDockerSolverBackend` implements the same boundary using asynchronous TCP and the standalone `Backend/V100` CUDA PointFocus server. The server computes the same FP64 PointFocus algorithm across all visible CUDA GPUs, with contiguous pixel portions and the full ordered emitter sum on each device; UE handles transport, job lifecycle, cancellation, and result reception. CGHV protocol 1.4 distinguishes constant-amplitude and inverse-distance computed results from the explicit `--solver dummy` transport-test mode. The default PointFocus arithmetic is preserved; the inverse-distance mode is opt-in. See the [Docker backend guide](CGH_Docker_Backend.md).
...
The solver defaults to **CPU**, **PointFocus**, and **Auto Solve disabled** (`bAutoSolve = false`). Both algorithm choices cover mesh clouds and multiple targets. In Details, choose **Parameters → Algorithm → Point Focus (1/r Amplitude)** to enable distance attenuation, or **PointFocus** to restore the original calculation. In Blueprint, use **Set Members in CGHSolverParameters** to set `Algorithm` to `PointFocusInverseR` or `PointFocus`, assign the updated struct to the actor's `Parameters`, then call `StartSolve` (**Start Solve**). The editor's **Generate Phase Pattern** button invokes the same operation. With Auto Solve enabled, changing the algorithm triggers another solve and invalidates an in-flight result from the previous mode.

| Control or state | Behavior |
| --- | --- |
| **Parameters → Algorithm** | `PointFocus` (default, constant amplitude) or `Point Focus (1/r Amplitude)`. Available to the editor and Blueprint for either backend. |
...
| **Auto Solve** | Opt-in mode: submits initially and when algorithm selection, consumed optical inputs, target list/references, or mesh resource revisions change. Target amplitude participates in the complex sum and now triggers recomputation. Cancelling does not immediately retry unchanged input. Restoring a temporarily missing reference allows automatic recovery; unchanged invalid numerical inputs, including NaNs, do not submit repeated jobs. Valid light position/amplitude/polarization changes, camera/debug appearance, and manual phase clear alone do not trigger recomputation; invalid fields still fail validation. |
...
`Ready` means phase data was published. It does not mean an optical reconstruction image or a measured physical focus has been produced. The camera can show ordinary Unreal geometry or a separate reconstructed optical field. Run **Camera (Thin Lens)** on the reconstructor to produce its sensor image; see the [camera guide](CGH_Camera_Reconstruction.md).
...
4. Start with a small SLM grid and choose **CPU** or **Docker (TCP/CUDA)** as the backend. Under **Parameters → Algorithm**, keep **PointFocus** for the existing behavior or select **Point Focus (1/r Amplitude)** for distance-weighted contributions. Click **Generate Phase Pattern**, or use **Solve Phase Pattern** on the linked workbench. Select the SLM to inspect the generated grayscale pattern after `Ready`.
...
**Inverse-distance amplitude mode — 2026-09-22, UE 5.8.2 on Linux:** Editor and Game Development builds and the CUDA 12.9.2 Docker image build passed. All **101/101 headless CGH tests passed**, with zero test warnings, failures, or skips. All **6/6 standalone CTest suites passed** using physical V100 GPUs 1 and 3. Tests check independent `A/r` complex-field oracles, exact single-emitter compatibility, changed multi-emitter phases, equal-distance equivalence, posed mesh samples, extreme amplitude/distance ranges, numerical zero-distance rejection, weighted destructive interference, cancellation, editor/Blueprint reflection, automatic mode changes, stale result rejection, and algorithm-specific TCP status validation. The largest wrapped CPU/CUDA difference in the new live inverse-distance fixtures was **2.8421709430404007e-14 radians** (tolerance `1e-7`); this is a fixture result, not a bound for arbitrary inputs. Single- and dual-GPU results were bit-identical across 65,536-pixel tiles and 256-emitter batches. All **15 existing Content files** remained byte-identical.

Report: `Saved/Automation/CGHInverseR/index.json`. Build, test, Docker, and kernel-resource logs: `Saved/Logs/CGHInverseR*_2026-09-22.log`. Launch sizes remain **65,536 pixels / 128 threads / 256 emitters**. The inverse-distance accumulation uses 64 registers/thread with no reported local/spill memory; the original accumulation remains at 54 registers/thread. This verifies numerical and lifecycle behavior, not a performance comparison or optimal configuration for the new mode.
