...
bool EncodeInput(const FWorkerContext& Context, std::vector<uint8_t>& Payload, FString& Error)
{
	...
	{
		Error = TEXT("Docker protocol 1.0 does not support this algorithm or propagation convention.");
		return false;
	}
	...
	uint64 RequestId, bool bSocketsAvailable)
{
	...
	// 1.0 result: fixed 32-byte metadata + one binary64 per pixel. Reject before allocating.
