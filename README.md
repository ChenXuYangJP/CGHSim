...
**Current milestone:** an asynchronous CPU reference solver generates a phase-only SLM pattern for multiple Point and Mesh targets by summing their complex fields, then compensating incident PlaneWave phase under the explicit `exp(+i k r)` propagation convention. The workbench supplies SI scene descriptions; the solver publishes to the existing SLM preview. An explicit Save button persists the current phase as a reusable Unreal asset, numerical files, and an exact-resolution grayscale PNG. PointFocus keeps the existing constant-amplitude behavior by default; the optional **Point Focus (1/r Amplitude)** mode weights each target contribution by inverse distance to the SLM pixel. Both are selectable in Details or Blueprint. A second Docker/TCP backend runs both modes across the selected V100 GPUs using CUDA 12.9.2. The standalone server also retains an explicit dummy mode for transport tests. A separate asynchronous CPU/CUDA reconstructor propagates the active SLM pattern onto an observer plane, retaining complex samples and providing phase/amplitude/intensity grayscale previews. The same reconstructor also propagates through a circular thin lens to the camera sensor, using focal length, f-number, focus distance, sensor sampling, and lens pose. The shared V100 Docker service handles observer and camera reconstruction alongside both PointFocus modes. The camera offers reconstructed optical and ordinary geometric previews.
...
| **Camera** | Thin-lens optical reconstruction with sensor resolution/pixel pitch, pupil sampling, complex field storage, Phase/Amplitude/Intensity previews, and explicit save/load. Existing sensor-size controls and a selectable Cine Camera geometric preview remain available. |
...
| **Reconstructor** | Asynchronous CPU or Docker/CUDA Rayleigh–Sommerfeld diffraction from the active SLM pattern and reconstruction light, cancellation, stale-result rejection, and optional automatic updates. A Save Complex Field button exports its accepted observer or camera result using the destination's save settings. The same CPU/Docker backend supports both observer and thin-lens camera modes. |
...
7. To inspect optical reconstruction, add **CGH Observer Plane Actor** and **CGH Reconstructor Actor**. Assign the observer on the workbench and the workbench on the reconstructor. Place the observer on the SLM-local **+X side**, start with small grids, and click **Reconstruct** with **CPU / Observer Plane** selected. Select the observer and choose **Amplitude**, **Phase**, or **Intensity** under **Preview Mode** in Details. After **Ready**, click **Save Complex Field** on the reconstructor or observer to save the numerical field and all three grayscale images; configure destinations on the observer. See the [reconstruction guide](Docs/CGH_Reconstruction.md) for the model, units, and asynchronous controls.
8. Save the level to preserve parameter and placement changes. Active phase and observer complex-field data remain transient. After reopening, select a saved asset in the SLM's **Stored Phase Pattern** and click **Load Stored Phase Pattern**, or use the observer's **Stored Complex Field** and **Load Stored Complex Field** with matching resolution and pixel pitches. You can also generate/reconstruct again. Saving does not change either stored-asset selection automatically. Generated reconstruction assets and raw/PNG outputs in the default save directories are ignored by Git.

For camera reconstruction, assign **Camera** on the workbench and choose reconstructor **Mode = Camera (Thin Lens)**. Set camera resolution, **Sensor Sampling = Pixel Pitch**, lens settings, and pupil resolution; select the camera after **Ready** for its default Intensity preview. See the [camera setup and sampling guide](Docs/CGH_Camera_Reconstruction.md).
...
For the optional Docker backend, see [Docker backend setup and verification](Docs/CGH_Docker_Backend.md). [Backend/V100](Backend/V100) builds independently of Unreal Engine using CMake project `CGHV100Backend` **1.4.0**, languages CXX/CUDA, and architecture 70. `src/solver/CudaPointFocus.cu` ports the existing CPU reference algorithm with separable compilation. **CGHV wire protocol 1.4** distinguishes constant-amplitude and inverse-distance PointFocus phases, observer and camera complex fields, and dummy results; rebuild both UE and server from this version.
...
- Camera parameters drive thin-lens reconstruction and the derived Cine Camera preview. Sensor resolution and effective pixel pitch define the optical output grid; camera +X looks toward the scene and its sensor lies behind the lens along -X. Coarse pupil sampling can alias; check convergence before interpreting sensor intensity.
...
**2026-09-22 — thin-lens camera reconstruction:** The existing reconstructor now handles camera sensors on CPU and Docker/CUDA, with sensor sampling, Phase/Amplitude/Intensity previews, optional geometric preview, and complex-field saving. Editor/Game and Docker builds passed; **116/116 Unreal tests**, **2/2 Vulkan preview tests**, and **8/8 standalone suites** passed. All 24 pre-existing Content files were preserved. See [camera setup, optics, sampling limits, and verification](Docs/CGH_Camera_Reconstruction.md).

**2026-09-22 — selectable 1/r solver amplitude:** CPU and Docker/CUDA support **Point Focus (1/r Amplitude)** alongside the original default. Editor/Game and Docker builds passed; **101/101 Unreal tests** and **6/6 standalone suites** passed, including single-point equality, independent multi-point/mesh numerics, mode switching, and multi-GPU parity. The V100 launch sizes and all existing Content files are preserved. See [solver verification](Docs/CGH_PointFocus_Solver.md#verification-record).

**2026-09-22 — Docker reconstruction:** The shared V100 service now handles observer-plane reconstruction with the preserved 64K-pixel/128-thread/256-contributor launch settings. Docker image and Editor/Game builds passed; **93/93 Unreal tests** and **6/6 standalone suites** passed on the two V100s. See [Docker reconstruction](Docs/CGH_Reconstruction.md#docker-reconstruction).
...
3. Extend camera sampling efficiency and detector response beyond the current direct thin-lens field model.
...
GS/FFT propagation, lens aberrations, sensor exposure/noise/color response, and runtime parameter UI remain future scope. See the handoff for proposed ordering and acceptance criteria.

## Documentation

- [Docker/TCP V100 backend](Docs/CGH_Docker_Backend.md) — independent CUDA PointFocus and reconstruction server, GPU selection, versioned protocol, numerical parity and transport verification.
- [Camera reconstruction](Docs/CGH_Camera_Reconstruction.md) — lens model, sensor/pupil sampling, camera orientation, CPU/Docker setup, previews, and persistence.
