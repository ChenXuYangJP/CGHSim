...
For the optional Docker backend, see [Docker backend setup and verification](Docs/CGH_Docker_Backend.md). [Backend/V100](Backend/V100) builds independently of Unreal Engine using CMake project `CGHV100Backend` **1.2.0**, languages CXX/CUDA, and architecture 70. `src/solver/CudaPointFocus.cu` ports the existing CPU reference algorithm with separable compilation. **CGHV wire protocol 1.2** distinguishes PointFocus phases, reconstructed complex fields, and dummy results; rebuild both UE and server from this version.
