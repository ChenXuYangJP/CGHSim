...
// CGHV 1.4: portable owned snapshots; no Unreal, CUDA, RTTI or exception dependency.
#include <atomic>
#include <cmath>
#include <cstddef>
#include <cstdint>
...
constexpr std::uint16_t kMajor = 1, kMinor = 4;
...
constexpr std::uint32_t kMaxPupilAxis = 2048, kMaxPupilPixels = 4194304;
constexpr std::size_t kMaxErrorBytes = 4096;
constexpr double kTwoPi = 6.283185307179586476925286766559;
enum class Type : std::uint16_t { Request = 1, Result = 2, Cancel = 3, Error = 4,
    ReconstructionRequest = 5, ReconstructionResult = 6, CameraReconstructionRequest = 7, CameraReconstructionResult = 8 };
enum class Algorithm : std::uint32_t { PointFocus = 1, ObserverPlaneReconstruction = 2, PointFocusInverseR = 3, CameraThinLens = 4 };
...
enum class Status : std::uint32_t { DummySuccess = 1, PointFocusSuccess = 2, ReconstructionSuccess = 3, PointFocusInverseRSuccess = 4, CameraReconstructionSuccess = 5 };
struct Header {
...
// Optical camera payload is independent of the legacy solver camera metadata.
struct CameraReconstruction {
    Vec3 optical_position_slm_m;
    Quat optical_rotation_slm;
    double focal_length_m = 0, f_number = 0, focus_distance_m = 0;
    std::uint32_t output_resolution_x = 0, output_resolution_y = 0;
    double pixel_pitch_x_m = 0, pixel_pitch_y_m = 0;
    std::uint32_t pupil_resolution_x = 0, pupil_resolution_y = 0;
};
struct CameraReconstructionRequest {
    Algorithm algorithm = Algorithm::CameraThinLens;
    Convention convention = Convention::ExpPositiveIKR;
    SLM slm;
    Light light;
    CameraReconstruction camera;
    std::vector<double> phase_radians;
};
struct CameraReconstructionResult {
    Status status = Status::CameraReconstructionSuccess;
    Convention convention = Convention::ExpPositiveIKR;
    std::uint32_t resolution_x = 0, resolution_y = 0;
    double compute_seconds = 0;
    std::vector<ComplexSample> samples;
...
inline bool ValidPupilDimensions(std::uint32_t x, std::uint32_t y) {
    return x > 0 && y > 0 && x <= kMaxPupilAxis && y <= kMaxPupilAxis && std::uint64_t(x) * y <= kMaxPupilPixels;
}
...
inline bool ValidateCameraReconstructionRequest(const CameraReconstructionRequest& request, std::string& error,
                                        const std::atomic<bool>* cancelled = nullptr) {
    error.clear();
    if (IsCancelled(cancelled)) return Fail(error, "Cancelled");
    if (request.algorithm != Algorithm::CameraThinLens || request.convention != Convention::ExpPositiveIKR)
        return Fail(error, "Unsupported reconstruction algorithm or propagation convention");
    const auto& s = request.slm;
    if (!ValidDimensions(s.resolution_x, s.resolution_y)) return Fail(error, "Invalid or oversized SLM dimensions");
    if (s.modulation != Modulation::PhaseOnly && s.modulation != Modulation::Complex) return Fail(error, "Unsupported modulation enum");
    ...
    if (!ValidReconstructionDimensions(c.output_resolution_x, c.output_resolution_y) ||
        !ValidPupilDimensions(c.pupil_resolution_x, c.pupil_resolution_y)) return Fail(error, "Invalid camera sensor or pupil dimensions");
    if (!IsFinite(c.optical_position_slm_m) || !IsFinite(c.optical_rotation_slm) ||
        !std::isfinite(c.focal_length_m) || c.focal_length_m <= 0 || !std::isfinite(c.f_number) || c.f_number <= 0 ||
        !std::isfinite(c.focus_distance_m) || c.focus_distance_m <= c.focal_length_m ||
        !std::isfinite(c.pixel_pitch_x_m) || c.pixel_pitch_x_m <= 0 ||
        !std::isfinite(c.pixel_pitch_y_m) || c.pixel_pitch_y_m <= 0) return Fail(error, "Invalid camera optical geometry");
    if (request.phase_radians.size() != std::uint64_t(s.resolution_x) * s.resolution_y) return Fail(error, "SLM phase count does not match dimensions");
    for (double phase : request.phase_radians) {
        if (IsCancelled(cancelled)) return Fail(error, "Cancelled");
        if (!std::isfinite(phase)) return Fail(error, "Nonfinite SLM phase value");
...
inline bool ValidateRequest(const Request& request, std::string& error, const std::atomic<bool>* cancelled = nullptr) {
    ...
    if ((request.algorithm != Algorithm::PointFocus && request.algorithm != Algorithm::PointFocusInverseR) ||
        request.convention != Convention::ExpPositiveIKR)
...
inline bool EncodeHeader(const Header& header, std::vector<std::uint8_t>& bytes, std::string& error) {
    ...
    if (header.type < Type::Request || header.type > Type::CameraReconstructionResult || !header.request_id || header.payload_size > kMaxPayloadBytes ||
...
inline bool ValidateResult(const Result& result, std::string& error, const std::atomic<bool>* cancelled = nullptr) {
    error.clear(); if ((result.status != Status::DummySuccess && result.status != Status::PointFocusSuccess && result.status != Status::PointFocusInverseRSuccess) || result.convention != Convention::ExpPositiveIKR) return Fail(error, "Unsupported result status or convention");
...
inline bool EncodeCameraReconstructionRequest(const CameraReconstructionRequest& request, std::vector<std::uint8_t>& bytes,
                                        std::string& error, const std::atomic<bool>* cancelled = nullptr) {
    bytes.clear();
    if (!ValidateCameraReconstructionRequest(request, error, cancelled)) return false;
    bytes.reserve(256 + request.phase_radians.size() * 8);
    detail::Writer w{bytes};
    w.U32(static_cast<std::uint32_t>(request.algorithm)); w.U32(static_cast<std::uint32_t>(request.convention));
    const auto& s = request.slm;
    w.U32(s.resolution_x); w.U32(s.resolution_y); w.F64(s.pixel_pitch_x_m); w.F64(s.pixel_pitch_y_m);
    ...
    w.U32(static_cast<std::uint32_t>(l.source)); w.V3(l.position_slm_m); w.F64(l.polarization_angle_rad);
    const auto& c = request.camera;
    w.V3(c.optical_position_slm_m); w.Q4(c.optical_rotation_slm);
    w.F64(c.focal_length_m); w.F64(c.f_number); w.F64(c.focus_distance_m);
    w.U32(c.output_resolution_x); w.U32(c.output_resolution_y);
    w.F64(c.pixel_pitch_x_m); w.F64(c.pixel_pitch_y_m);
    w.U32(c.pupil_resolution_x); w.U32(c.pupil_resolution_y);
    w.U64(request.phase_radians.size());
    for (double phase : request.phase_radians) {
        if (IsCancelled(cancelled)) { bytes.clear(); return Fail(error, "Cancelled"); }
        w.F64(phase);
...
inline bool DecodeCameraReconstructionRequest(const std::uint8_t* bytes, std::size_t size, CameraReconstructionRequest& request,
                                        std::string& error, const std::atomic<bool>* cancelled = nullptr) {
    error.clear();
    if (!bytes || size < 256 || size > kMaxPayloadBytes) return Fail(error, "Invalid reconstruction request size");
    if (IsCancelled(cancelled)) return Fail(error, "Cancelled");
    detail::Reader r{bytes, size}; CameraReconstructionRequest q;
    q.algorithm = static_cast<Algorithm>(r.U32()); q.convention = static_cast<Convention>(r.U32());
    auto& s = q.slm;
    s.resolution_x = r.U32(); s.resolution_y = r.U32(); s.pixel_pitch_x_m = r.F64(); s.pixel_pitch_y_m = r.F64();
    s.active_width_m = r.F64(); s.active_height_m = r.F64(); s.modulation = static_cast<Modulation>(r.U32());
    ...
    l.source = static_cast<Source>(r.U32()); l.position_slm_m = r.V3(); l.polarization_angle_rad = r.F64();
    auto& c = q.camera;
    c.optical_position_slm_m = r.V3(); c.optical_rotation_slm = r.Q4();
    c.focal_length_m = r.F64(); c.f_number = r.F64(); c.focus_distance_m = r.F64();
    c.output_resolution_x = r.U32(); c.output_resolution_y = r.U32();
    c.pixel_pitch_x_m = r.F64(); c.pixel_pitch_y_m = r.F64();
    c.pupil_resolution_x = r.U32(); c.pupil_resolution_y = r.U32();
    const auto count = r.U64();
    // Check both geometry and exact remaining bytes before allocating the numerical array.
    if (!ValidDimensions(s.resolution_x, s.resolution_y) || !ValidReconstructionDimensions(c.output_resolution_x, c.output_resolution_y) ||
        !ValidPupilDimensions(c.pupil_resolution_x, c.pupil_resolution_y) ||
        count != std::uint64_t(s.resolution_x) * s.resolution_y || count > kMaxPixels ||
        !r.Have(static_cast<std::size_t>(count) * 8) || size - r.offset != count * 8)
        return Fail(error, "Truncated, oversized or mismatched reconstruction phase array");
    q.phase_radians.resize(static_cast<std::size_t>(count));
    ...
    if (!r.Done()) return Fail(error, "Truncated reconstruction request or trailing bytes");
    if (!ValidateCameraReconstructionRequest(q, error, cancelled)) return false;
    request = std::move(q); return true;
}
inline bool ValidateCameraReconstructionResult(const CameraReconstructionResult& result, std::string& error,
                                         const std::atomic<bool>* cancelled = nullptr) {
    error.clear();
    if (IsCancelled(cancelled)) return Fail(error, "Cancelled");
    if (result.status != Status::CameraReconstructionSuccess || result.convention != Convention::ExpPositiveIKR)
        return Fail(error, "Unsupported reconstruction status or convention");
    if (!ValidReconstructionDimensions(result.resolution_x, result.resolution_y) ||
        result.samples.size() != std::uint64_t(result.resolution_x) * result.resolution_y)
        return Fail(error, "Invalid reconstruction result dimensions or sample count");
...
inline bool EncodeCameraReconstructionResult(const CameraReconstructionResult& result, std::vector<std::uint8_t>& bytes,
                                       std::string& error, const std::atomic<bool>* cancelled = nullptr) {
    bytes.clear();
    if (!ValidateCameraReconstructionResult(result, error, cancelled)) return false;
    bytes.reserve(32 + result.samples.size() * 16);
    detail::Writer w{bytes};
    w.U32(static_cast<std::uint32_t>(result.status)); w.U32(static_cast<std::uint32_t>(result.convention));
    w.U32(result.resolution_x); w.U32(result.resolution_y); w.F64(result.compute_seconds); w.U64(result.samples.size());
...
inline bool DecodeCameraReconstructionResult(const std::uint8_t* bytes, std::size_t size, CameraReconstructionResult& result,
                                       std::string& error, const std::atomic<bool>* cancelled = nullptr) {
    error.clear();
    if (!bytes || size < 32 || size > kMaxPayloadBytes) return Fail(error, "Invalid reconstruction result size");
    if (IsCancelled(cancelled)) return Fail(error, "Cancelled");
    detail::Reader r{bytes, size}; CameraReconstructionResult q;
    q.status = static_cast<Status>(r.U32()); q.convention = static_cast<Convention>(r.U32());
    q.resolution_x = r.U32(); q.resolution_y = r.U32(); q.compute_seconds = r.F64(); const auto count = r.U64();
    if (!ValidReconstructionDimensions(q.resolution_x, q.resolution_y) ||
        count != std::uint64_t(q.resolution_x) * q.resolution_y || count > kMaxReconstructionPixels ||
    ...
    if (!r.Done()) return Fail(error, "Truncated reconstruction result or trailing bytes");
    if (!ValidateCameraReconstructionResult(q, error, cancelled)) return false;
    result = std::move(q); return true;
}
