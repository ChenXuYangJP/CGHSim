...
enum class Algorithm : std::uint32_t { PointFocus = 1 };
...
inline bool EncodeHeader(const Header& header, std::vector<std::uint8_t>& bytes, std::string& error) {
    ...
    if (header.type < Type::Request || header.type > Type::Error || !header.request_id || header.payload_size > kMaxPayloadBytes ||
